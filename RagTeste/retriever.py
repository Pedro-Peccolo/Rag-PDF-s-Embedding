...
def testar_conectividade_procempa():
    ...
    try:
        # Tentar fazer uma requisição POST de teste para ver se a API responde
        payload = {
            "input": "teste",
...
def buscar_com_scores(db, pergunta, k=3, a=1.0):
    """Busca documentos e calcula scores híbridos (embedding + BM25) com PROCEMPA"""
    print(f"\nPergunta: {pergunta}")
    print(f"Calculando scores híbridos (a={a:.1f}: {a*100:.0f}% embedding, {(1-a)*100:.0f}% BM25)...\n")

    # Usar busca híbrida completa com PROCEMPA
    return buscar_com_scores_hibrido(db, pergunta, k, a)

def buscar_com_scores_hibrido(db, pergunta, k=3, a=1.0):
    """Busca híbrida completa com embeddings PROCEMPA"""
    ...
    print("Embedding da query gerado (PROCEMPA)")
